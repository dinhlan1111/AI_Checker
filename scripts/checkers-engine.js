...
//Trả về 1 object gồm cells, pieces, turn chứa các trạng thái của bàn cờ
//cell, piece: row, col, state
//Nhưng cell lưu toàn bộ các state của bàn cờ 
//Piece chỉ lưu các trạng thái !Empty
//Turn: trả về red
...
// Ghi biên bản thi đấu writePaper
function writePaper(){
	var bienBan=""
	...
	for(var i = 0; i <allMoves.length; i++){
		...
		if (nextMove.to.row > -1){
			bienBan += "From "+nextMove.from.col+" : "+nextMove.from.row+"  -   ";
			bienBan += "To "+ nextMove.to.col+" : "+nextMove.to.row ;
		}
		else{
			bienBan += "An: " + nextMove.from.col + " : "+ nextMove.from.row ;
		}
		bienBan+="\n"
	}
	document.getElementById("myTextarea").innerHTML = bienBan;
}

//Relayall trận đấu khi đánh xong
...
function redo(){
	var redoMove = redoStep;
	var allMoves = redoMove;
	for (var i=allMoves.length; i>=0; i--) {
		var moveNum = i-1;
		var nextMove = allMoves[i];
		if (nextMove == null){
			console.log("bo qua")
...
function dragEnded(origin, width, node, d) {
	...
	if (!legal || d.state === black || d.state === blackKing) {
...
function drawBoard(origin, cellWidth, boardCanvas) {
	...
	boardCanvas.append("g")
				...
				.data(cells)
				.enter().append("rect")
				.attr("x", function(d) { return mapCellToCoordinates(origin, cellWidth, d).x; return x+cellWidth/2;})
				.attr("y", function(d) { return mapCellToCoordinates(origin, cellWidth, d).y; return y+cellWidth/2;})
...
function drawText(data) {
	boardCanvas.append("g")
				...
				.text(function(d) { /*if (d.state === red) return "R"; 
									...
									else*/ if (d.state === redKing || d.state === blackKing) return "KO";
									else return "";})
				;
}
...
function computerMove() {
    ...
    document.getElementById("time").innerHTML = "Time: " + moveTime;
